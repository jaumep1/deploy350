...
export default function SignupLayout() {
  ...
  const handleSubmit = async (event) => {
    if (event.key === 'Enter' || event.target.id === 'submit') {
      if (firstName.length === 0) {
        toast.error('First name must be filled', { duration: 1000 });
      } else if (lastName.length === 0) {
        toast.error('Last name must be filled', { duration: 1000 });
      } else if (email.length === 0) {
        toast.error('Email must be filled', { duration: 1000 });
      } else if (password.length === 0) {
        toast.error('Password must be filled', { duration: 1000 });
      } else if (role.length === 0) {
        toast.error('Role must be filled', { duration: 1000 });
      } else {
        ...
        fetch(`${domain}/signup`, {
          ...
          if (res.status === 201) {
            ...
            toast.error('Failed signup, please try again', { duration: 1000 });
          }
        });
      }
    }
  ...
  return (
    <div style={{ height: '100vh', padding: '5vh' }} className="w-100">
      <div
        className="d-flex mb-[10vh] w-fit mx-auto justify-content-center"
      >
      ...
      <div className="text-center text-2xl font-semibold">Pick.It Signup</div>
      <div className="mx-auto w-[50vw]">
        <div className="flex mx-auto w-[30vw]">
          <div className="relative my-4 mr-2 w-full">
            <input
              type="text"
              id="firstNameInput"
            ...
            />
            <label className="pointer-events-none bg-white absolute text-gray-500 transform -translate-y-0 duration-100 top-2 scale-75 z-10 origin-[0] bg-whitepx-2 peer-focus:px-2 peer-focus:text-teal-500 peer-placeholder-shown:scale-100 peer-placeholder-shown:-translate-y-[3px] peer-placeholder-shown:top-1/2 peer-focus:top-2 peer-focus:scale-75 peer-focus:translate-y-0 left-4 peer-focus:left-2">First Name</label>
          </div>
          <div className="relative my-4 ml-2 w-full">
            <input
              type="text"
              id="lastNameInput"
              className="block px-2.5 pb-3 py-4 mt-4 pl-3 w-full border border-1 text-gray-900 bg-transparent rounded border-gray-200 appearance-none focus:outline-none focus:ring-0 focus:border-teal-500 peer"
              ...
              onKeyDown={handleSubmit}
            />
            <label className="pointer-events-none bg-white absolute text-gray-500 transform -translate-y-0 duration-100 top-2 scale-75 z-10 origin-[0] bg-whitepx-2 peer-focus:px-2 peer-focus:text-teal-500 peer-placeholder-shown:scale-100 peer-placeholder-shown:-translate-y-[3px] peer-placeholder-shown:top-1/2 peer-focus:top-2 peer-focus:scale-75 peer-focus:translate-y-0 left-4 peer-focus:left-2">Last Name</label>
          </div>
        </div>
        <div className="relative mx-auto w-[30vw]">
          <input
            type="text"
            id="emailInput"
            className="block px-2.5 pb-3 py-4 pl-3 w-full border border-1 text-gray-900 bg-transparent rounded border-gray-200 appearance-none focus:outline-none focus:ring-0 focus:border-teal-500 peer"
            placeholder=" "
            onChange={(e) => setEmail(e.target.value)}
            onKeyDown={handleSubmit}
          />
          <label htmlFor="floating_outlined" className="pointer-events-none bg-white absolute text-gray-500 duration-100 transform -translate-y-4 scale-75 top-2 z-10 origin-[0] bg-whitepx-2 peer-focus:px-2 peer-focus:text-teal-500 peer-placeholder-shown:scale-100 peer-placeholder-shown:-translate-y-1/2 peer-placeholder-shown:top-1/2 peer-focus:top-2 peer-focus:scale-75 peer-focus:-translate-y-4 left-4 peer-focus:left-2">Email</label>
        </div>
        <div className="relative mx-auto w-[30vw]">
          <input
            type="password"
        ...
        <div className="w-fit mx-auto mt-4">
          <div className="text-gray-500 mb-2 ml-3">Role</div>
          <div className="flex w-fit ml-3">
            <div className="flex items-center mr-2">
              <button id="nurseButton" type="button" onClick={() => setRole('Nurse')} className={`${role === 'Nurse' ? 'bg-teal-600 hover:bg-teal-500' : 'bg-gray-300 hover:bg-gray-200 border border-1 border-gray-400'} duration-150 w-4 h-4 text-teal-600 rounded`}>
                { role === 'Nurse' && (
              ...
              <div className="ml-1 text-sm font-medium text-gray-900 mr-4">Nurse</div>
              <button type="button" onClick={() => setRole('Storeroom')} className={`${role === 'Storeroom' ? 'bg-teal-600 hover:bg-teal-500 ' : 'bg-gray-300 hover:bg-gray-200 border border-1 border-gray-400'} duration-150 w-4 h-4 text-teal-600 rounded`}>
                { role === 'Storeroom' && (
                  <svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" fill="white" className="bi bi-check" viewBox="0 0 16 16">
                    <path d="M10.97 4.97a.75.75 0 0 1 1.07 1.05l-3.99 4.99a.75.75 0 0 1-1.08.02L4.324 8.384a.75.75 0 1 1 1.06-1.06l2.094 2.093 3.473-4.425a.267.267 0 0 1 .02-.022z" />
                  </svg>
              ...
              <div className="ml-1 text-sm font-medium text-gray-900 ">Storeroom</div>
            </div>
          </div>
        </div>
      </div>
      <div className="m-auto text-center mb-3 mt-4 w-[30vw]">
        <button
          type="button"
          id="submit"
          onClick={(e) => handleSubmit(e)}
          className="text-white w-full bg-teal-600 mt-4 active:scale-95 hover:bg-teal-500 duration-150 font-medium rounded px-5 py-4 focus:outline-none"
        >
          Continue
        </button>
      </div>
      <p className="m-auto text-center text-sm">
        ...
        {' '}
        <a href="/login" className="text-teal-500 no-underline hover:underline">
          Log in
        </a>
      </p>
    </div>
